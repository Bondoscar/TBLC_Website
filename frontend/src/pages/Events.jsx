...
const Events = () => {
  ...
  return (
    <div className="bg-blue-950 text-white" data-testid="events-page">
      ...
      <section className="py-16 px-6 lg:px-10">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {events.map((e) => (
            <div key={e.id} className="border-2 border-white/20 card-hover overflow-hidden" data-testid={`event-card-${e.id}`}>
              <div className="aspect-[16/9] overflow-hidden">
                <img src={imgFrom(e)} alt={e.title} className="w-full h-full object-cover" />
              </div>
              <div className="p-7">
                <h3 className="serif-display text-2xl font-semibold">{e.title}</h3>
                <div className="mt-3 flex flex-wrap gap-4 text-sm text-white/70">
                  {e.date && <span className="flex items-center gap-2"><Calendar size={14} /> {e.date}</span>}
                  {e.time && <span className="flex items-center gap-2"><Clock size={14} /> {e.time}</span>}
                </div>
